 ...
 *  1:1 port of v1 HomeSanctuary.tsx (static elements only — animated parts in drawCampfire). */
export function homeSanctuarySVG(): string {
  ...
  <!-- ══ STONE FOUNDATION ══ -->
  <polygon points="-5,${GY} 0,${GY - 6} 105,${GY - 6} 110,${GY} 110,${GY + 3} -5,${GY + 3}" fill="${STONE_DARK}" opacity="0.7"/>
  ...
  <line x1="20" y1="${GY - 5}" x2="30" y2="${GY - 1}" stroke="#3a3028" stroke-width="0.7" opacity="0.3"/>
  <line x1="70" y1="${GY - 6}" x2="78" y2="${GY - 2}" stroke="#3a3028" stroke-width="0.5" opacity="0.25"/>
  <line x1="50" y1="${GY - 4}" x2="55" y2="${GY}" stroke="#3a3028" stroke-width="0.4" opacity="0.2"/>

  <!-- ══ BACK WALL (ruined, tall) ══ -->
  <polygon points="0,${GY - 6} 0,${GY - 65} 6,${GY - 70} 14,${GY - 68} 20,${GY - 72} 28,${GY - 66} 35,${GY - 70} 42,${GY - 64} 45,${GY - 6}" fill="${STONE_DARK}" opacity="0.6"/>
  <polygon points="3,${GY - 6} 3,${GY - 62} 10,${GY - 67} 18,${GY - 64} 22,${GY - 68} 30,${GY - 6}" fill="${STONE}" opacity="0.25"/>
  <!-- Mortar block lines -->
  ${[-55, -45, -35, -25, -16].map(dy =>
    `<line x1="2" y1="${GY + dy}" x2="43" y2="${GY + dy}" stroke="${MORTAR}" stroke-width="0.5" opacity="0.3"/>`
  ).join('')}
  ${[10, 22, 33].map(dx => `
    <line x1="${dx}" y1="${GY - 55}" x2="${dx}" y2="${GY - 45}" stroke="${MORTAR}" stroke-width="0.4" opacity="0.25"/>
    <line x1="${dx + 6}" y1="${GY - 35}" x2="${dx + 6}" y2="${GY - 25}" stroke="${MORTAR}" stroke-width="0.4" opacity="0.25"/>
  `).join('')}
  ...
  <path d="M8,${GY - 8} Q6,${GY - 20} 10,${GY - 35} Q12,${GY - 45} 8,${GY - 55}" stroke="#3a5a2a" stroke-width="1" fill="none" opacity="0.4"/>
  ...
  <!-- Crack down pillar -->
  <line x1="2" y1="${GY - 60}" x2="3" y2="${GY - 40}" stroke="#3a3028" stroke-width="0.7" opacity="0.35"/>
  ...
  <!-- Jagged broken top -->
  <polygon points="98,${GY - 40} 99,${GY - 44} 101,${GY - 41} 103,${GY - 46} 105,${GY - 42} 106,${GY - 40}" fill="${STONE}" opacity="0.7"/>
  <!-- Moss on broken pillar -->
  <ellipse cx="102" cy="${GY - 20}" rx="2" ry="1" fill="${MOSS}" opacity="0.35"/>
  <!-- Fallen chunks -->
  <rect x="108" y="${GY - 4}" width="6" height="5" fill="${STONE_DARK}" opacity="0.4" rx="1" transform="rotate(15, 111, ${GY - 1})"/>
  <rect x="112" y="${GY - 2}" width="4" height="3" fill="${STONE}" opacity="0.3" rx="0.5" transform="rotate(-10, 114, ${GY})"/>
  <circle cx="118" cy="${GY - 1}" r="1.5" fill="${STONE_DARK}" opacity="0.25"/>

  <!-- ══ LEAN-TO ROOF ══ -->
  <!-- Main beam -->
  <line x1="2" y1="${GY - 72}" x2="95" y2="${GY - 45}" stroke="${WOOD}" stroke-width="3" stroke-linecap="round" opacity="0.8"/>
  <!-- Right support post -->
  <line x1="95" y1="${GY - 45}" x2="95" y2="${GY - 5}" stroke="${WOOD_DARK}" stroke-width="2.5" opacity="0.65"/>
  <!-- Cross brace -->
  <line x1="85" y1="${GY - 5}" x2="95" y2="${GY - 30}" stroke="${WOOD_DARK}" stroke-width="1.5" opacity="0.4"/>
  <!-- Hide/canvas covering -->
  <polygon points="2,${GY - 69} 92,${GY - 43} 92,${GY - 35} 2,${GY - 60}" fill="${HIDE}" opacity="0.45"/>
  <polygon points="2,${GY - 66} 88,${GY - 42} 88,${GY - 38} 2,${GY - 62}" fill="#7a6a58" opacity="0.2"/>
  <!-- Patch on canvas -->
  <rect x="40" y="${GY - 56}" width="10" height="7" fill="#5a4838" opacity="0.3" rx="1" transform="rotate(-18, 45, ${GY - 52})"/>
  <!-- Rope lashings -->
  <line x1="18" y1="${GY - 69}" x2="20" y2="${GY - 66}" stroke="${ROPE}" stroke-width="0.8" opacity="0.5"/>
  <line x1="45" y1="${GY - 59}" x2="47" y2="${GY - 56}" stroke="${ROPE}" stroke-width="0.8" opacity="0.5"/>
  <line x1="72" y1="${GY - 50}" x2="74" y2="${GY - 47}" stroke="${ROPE}" stroke-width="0.8" opacity="0.5"/>

  <!-- ══ BANNER (static base — animation in drawCampfire) ══ -->
  <line x1="2" y1="${GY - 80}" x2="2" y2="${GY - 82}" stroke="${WOOD_DARK}" stroke-width="1.5" opacity="0.6"/>
  <line x1="-3" y1="${GY - 80}" x2="7" y2="${GY - 80}" stroke="${WOOD}" stroke-width="1.2" opacity="0.6"/>
  ...
  ${[0, 45, 90, 135, 180, 225, 270, 315].map(angle => {
    ...
    return `<ellipse cx="${cx}" cy="${cy}" rx="2" ry="1.2" fill="${STONE}" opacity="0.5"/>`;
  }).join('')}
  <!-- Ash bed -->
  <ellipse cx="60" cy="${GY - 2}" rx="5" ry="2" fill="#3a3030" opacity="0.4"/>
  <!-- Logs -->
  <line x1="56" y1="${GY - 2}" x2="64" y2="${GY - 5}" stroke="${WOOD_DARK}" stroke-width="2.5" stroke-linecap="round" opacity="0.7"/>
  <line x1="57" y1="${GY - 5}" x2="63" y2="${GY - 1}" stroke="${WOOD_DARK}" stroke-width="2" stroke-linecap="round" opacity="0.6"/>
  <line x1="58" y1="${GY - 1}" x2="62" y2="${GY - 4}" stroke="#3a2010" stroke-width="1.5" stroke-linecap="round" opacity="0.5"/>

  <!-- ══ COOKING SPIT ══ -->
  <!-- Forked sticks -->
  <line x1="53" y1="${GY - 2}" x2="53" y2="${GY - 18}" stroke="${WOOD_DARK}" stroke-width="1.2" opacity="0.6"/>
  <line x1="52" y1="${GY - 17}" x2="54" y2="${GY - 19}" stroke="${WOOD_DARK}" stroke-width="0.8" opacity="0.5"/>
  <line x1="67" y1="${GY - 2}" x2="67" y2="${GY - 16}" stroke="${WOOD_DARK}" stroke-width="1.2" opacity="0.6"/>
  <line x1="66" y1="${GY - 15}" x2="68" y2="${GY - 17}" stroke="${WOOD_DARK}" stroke-width="0.8" opacity="0.5"/>
  <!-- Cross bar -->
  <line x1="53" y1="${GY - 17}" x2="67" y2="${GY - 15}" stroke="${WOOD}" stroke-width="1" opacity="0.6"/>
  <!-- Pot -->
  <ellipse cx="60" cy="${GY - 14}" rx="3.5" ry="1" fill="${METAL_DARK}" opacity="0.5"/>
  <path d="M56.5,${GY - 14} Q56,${GY - 10} 60,${GY - 9} Q64,${GY - 10} 63.5,${GY - 14}" fill="${METAL_DARK}" opacity="0.55"/>

  <!-- ══ WEAPON RACK ══ -->
  <line x1="80" y1="${GY - 5}" x2="80" y2="${GY - 30}" stroke="${WOOD}" stroke-width="2" opacity="0.7"/>
  <line x1="90" y1="${GY - 5}" x2="90" y2="${GY - 30}" stroke="${WOOD}" stroke-width="2" opacity="0.7"/>
  <line x1="79" y1="${GY - 26}" x2="91" y2="${GY - 26}" stroke="${WOOD}" stroke-width="1.2" opacity="0.6"/>
  <line x1="79" y1="${GY - 15}" x2="91" y2="${GY - 15}" stroke="${WOOD}" stroke-width="1.2" opacity="0.6"/>
  <!-- Sword -->
  <line x1="83" y1="${GY - 28}" x2="83.5" y2="${GY - 10}" stroke="${METAL}" stroke-width="1.5" opacity="0.55"/>
  <line x1="81" y1="${GY - 28}" x2="86" y2="${GY - 28}" stroke="${METAL}" stroke-width="1.2" opacity="0.45"/>
  <circle cx="83.5" cy="${GY - 28}" r="0.8" fill="${BANNER_TRIM}" opacity="0.4"/>
  <!-- Axe -->
  <line x1="87" y1="${GY - 27}" x2="87.5" y2="${GY - 10}" stroke="${WOOD_LIGHT}" stroke-width="1.2" opacity="0.5"/>
  <polygon points="85,${GY - 26} 87,${GY - 28} 89,${GY - 26} 88,${GY - 23}" fill="${METAL}" opacity="0.45"/>
  ...
  <line x1="92" y1="${GY - 20}" x2="92" y2="${GY - 8}" stroke="#3a4a6a" stroke-width="0.6" opacity="0.3"/>
  <line x1="88.5" y1="${GY - 14}" x2="95.5" y2="${GY - 14}" stroke="#3a4a6a" stroke-width="0.6" opacity="0.3"/>

  <!-- ══ HANGING LANTERN (static frame — animation in drawCampfire) ══ -->
  <line x1="35" y1="${GY - 56}" x2="35" y2="${GY - 48}" stroke="${METAL_DARK}" stroke-width="0.5" opacity="0.5"/>
  <rect x="33" y="${GY - 48}" width="4" height="6" fill="${METAL_DARK}" opacity="0.5" rx="0.5"/>
  ...
  <line x1="42" y1="${GY - 12}" x2="42" y2="${GY - 4}" stroke="${WOOD_DARK}" stroke-width="0.5" opacity="0.3"/>
  <line x1="38" y1="${GY - 8}" x2="46" y2="${GY - 8}" stroke="${WOOD_DARK}" stroke-width="0.5" opacity="0.3"/>
  <!-- Barrel -->
  ...
  <line x1="44" y1="${GY - 11}" x2="52" y2="${GY - 11}" stroke="${METAL_DARK}" stroke-width="0.6" opacity="0.3"/>
  <line x1="44" y1="${GY - 8}" x2="52" y2="${GY - 8}" stroke="${METAL_DARK}" stroke-width="0.6" opacity="0.3"/>
  <!-- Supply sack -->
  <ellipse cx="34" cy="${GY - 5}" rx="3.5" ry="3" fill="#7a6a4a" opacity="0.45"/>
  <line x1="34" y1="${GY - 7}" x2="34" y2="${GY - 9}" stroke="${ROPE}" stroke-width="0.7" opacity="0.4"/>

  <!-- ══ SCATTERED RUBBLE ══ -->
  <circle cx="-8" cy="${GY - 1}" r="2" fill="${STONE_DARK}" opacity="0.3"/>
  <rect x="-12" y="${GY - 2}" width="4" height="3" fill="${STONE}" opacity="0.25" rx="0.5" transform="rotate(20, -10, ${GY})"/>
